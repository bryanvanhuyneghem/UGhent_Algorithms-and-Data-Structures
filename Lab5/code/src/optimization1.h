...
bool BinaryTree::voegToe(std::vector<std::string> &questions, std::list<std::string> &animals,
                         std::vector<std::map<std::string, bool>> &answers) {
    if ((this->get()->question <= questions.size()) && animals.size() > 1) {
        ...
        this->get()->value = questions[this->get()->question]; // Set the question (value) for this Node in the Binary Tree
        ...
        this->get()->left->question = this->get()->question + 1;
        this->get()->right->question = this->get()->question + 1;
